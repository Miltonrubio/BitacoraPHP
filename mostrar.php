...
$fecha_y_hora_actual = date("Y-m-d H:i:s");
    ...
    // Opción 2: Obtener actividades del usuario
    $sql = "SELECT * FROM `nombre_actividades` ORDER BY tipo_actividad DESC ";
...
LEFT JOIN (
    SELECT
        ID_usuario,
        MAX(ID_saldo) AS ultimo_saldo_id
    FROM
        saldo
    WHERE
        status_saldo = 'activo'
...
s.ID_usuario,
s.status_saldo,
s.saldo - COALESCE(SUM(g.dinero_gastado), 0) AS saldo_actualizado
FROM 
saldo s
LEFT JOIN 
